...
const LBOBCharacter = ({ isTyping, size = 40 }) => {
...
// Message Component
const MessageItem = ({ message, isTyping }) => {
    ...
    <View style={[styles.messageContainer, isUser && styles.userMessageContainer]}>
      ...
      {isUser && (
        <View style={styles.userAvatarContainer}>
          <LinearGradient colors={['#4CAF50', '#45a049']} style={styles.userAvatar}>
...
const ChatScreen = ({ conversation, onBack }) => {
  ...
  return (
    <SafeAreaView style={styles.chatContainer}>
      ...
      {isTyping && (
        <MessageItem
          message={{ sender_type: 'ai', message_text: '' }}
          isTyping={true}
        />
      )}
...
const ConversationsScreen = ({ onSelectConversation, onNewConversation }) => {
  ...
  const createNewConversation = async () => {
    try {
      const conversation = await apiService.createConversation('New Troubleshooting Session');
